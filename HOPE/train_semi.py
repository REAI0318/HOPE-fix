    ...
    for epoch in range(args.epochs):
        ...
        for idx in range(niters_per_epoch):
            ...
            if args.use_low_confidence_correction:
                ...
                # 2. 获取修正后的标签 (利用多模态一致性)
                corrected_img1 = get_corrected_label(logits_img_tea_1, logits_pt_tea_1)
                corrected_img2 = get_corrected_label(logits_img_tea_2, logits_pt_tea_2)
                corrected_pt1 = get_corrected_label(logits_pt_tea_1, logits_img_tea_1)
                corrected_pt2 = get_corrected_label(logits_pt_tea_2, logits_img_tea_2)
            ...
            if iteration % args.per_print == 0:
                ...
                print("loss: %f  crc_loss: %f align_loss: %f cps_loss: %f quan_loss: %f corr_loss: %f semi_mg_loss: %f  global_loss: %f " % (loss.item(),crc_loss.item(), align_loss.item(), cps_loss.item(),quan_loss.item(), correction_loss.item(), local_semi_mg_loss.item(), global_mg_loss))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ma_beta', type=float, default=0.99, help='MA decay rate')
    parser.add_argument('--ma_warmup', type=int, default=8, help='MA warm-up epochs')
    parser.add_argument('--use_low_confidence_correction', action='store_true', help='use correction loss for low confidence samples')
    parser.add_argument('--weight_correction', type=float, default=0.3, help='weight for correction loss')
